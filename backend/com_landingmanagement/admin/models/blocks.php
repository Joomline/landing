 ...
 * @author Joomline
 */
class LandingmanagementModelBlocks extends JModelList
{
	...
	protected function populateState( $ordering = null, $direction = null )
	{
		...
		$block_type = $this->getUserStateFromRequest( $this->context . '.filter.block_type', 'filter_block_type' );
		$this->setState( 'filter.block_type', $block_type );
	...
	protected function getStoreId( $id = '' )
	{
		...
		$id .= ':' . $this->getState( 'filter.block_type' );
	...
	protected function getListQuery()
	{
		...
		$query->select( 't1.id, t1.title, t1.alias, t1.published as state, t1.created, t1.ordering, t1.type, t1.site_id' );
		$query->select( 'u.username as created_by, u.id as author_id' );
		$query->join( 'LEFT', '#__users AS u ON u.id = t1.created_by' );
		$query->from( '#__landingmanagement_blocks as t1' );
		...
		$block_type = $this->getState( 'filter.block_type' );
		if ( !empty($block_type) ) {
			$query->where( 't1.type=' . $this->_db->quote($block_type) );
		}
	...
	public function getAuthors()
	{
		...
		$query->join( 'INNER', '#__landingmanagement_blocks AS c ON c.created_by = u.id' );
