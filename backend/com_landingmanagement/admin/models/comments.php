 ...
 * @author Joomline
 */
class LandingmanagementModelComments extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		$query->select( 't1.id, t1.title, t1.published as state, t1.created, t1.ordering, t1.site_id' );
		$query->select( 'u.username as created_by, u.id as author_id' );
		$query->join( 'LEFT', '#__users AS u ON u.id = t1.created_by' );
		$query->from( '#__landingmanagement_comments as t1' );
		...
		$siteId = (int)$this->getState( 'filter.site_id', 0 );
		if ( $siteId > 0 ) {
			$query->where( 't1.site_id=' . $siteId );
		}
	...
	public function getAuthors()
	{
		...
		$query->join( 'INNER', '#__landingmanagement_comments AS c ON c.created_by = u.id' );
